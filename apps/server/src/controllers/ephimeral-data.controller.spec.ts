...
describe('EphimeralDataController (e2e)', () => {
  ...
  it('should get data', async () => {
    ...
    const responsePost = await request(app.getHttpServer()).post('/status').expect(200);
  ...
  it('should return 404 if data does not exists', async () => {
    await request(app.getHttpServer()).get('/fake').expect(404);

    await request(app.getHttpServer()).post('/fake').expect(404);
  });
